...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	// Store the textures
	textureName = { "asteroid1", "asteroid2", "asteroid3", "bullet","theRocket","theBackground"};
	texturesToUse = { "Images\\asteroid1.png", "Images\\asteroid2.png", "Images\\asteroid3.png", "Images\\bullet.png", "Images\\rocketSprite.png", "Images\\starscape1024x768.png"};
	...
	for (int astro = 0; astro < 5; astro++)
	{
		...
		theAsteroids[astro]->setSpritePos({ 100 * (rand() % 10 + 1), 50 * (rand() % 6 + 1) });
		theAsteroids[astro]->setSpriteTranslation({ (rand() % 16 + 1), (rand() % 10 + 1) });
	...
	while (SDL_PollEvent(&event))
	{
		...
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
				...
				case SDLK_w:
				{
					if (theRocket.getSpritePos().y > 0, theRocket.getSpritePos().x > 0)
					{
						theRocket.getSpriteRotAngle();
						theRocket.setRocketVelocity({ 0, -10 });
